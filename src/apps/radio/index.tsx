      ...
      {bars.map((h, i) => (
        <span
          ...
          style={{
            ...
            display: "inline-block", width: 8, textAlign: "center", fontSize: 10, lineHeight: 1,
          }}
        >
...
function TrayBtn({ onClick, children, title }: { onClick: () => void; children: React.ReactNode; title?: string }) {
  return (
    <button
      ...
      style={{
        backgroundColor: "var(--t-bg)", color: "var(--t-text)", fontFamily: "var(--t-font-display)",
...
export function RadioApp(_: AppProps) {
  ...
  return (
    <div className="flex flex-col h-full" style={{ fontFamily: "var(--t-font-display)", minHeight: 0 }}>
      <div className="shrink-0 p-4 border-b-2 flex flex-col gap-3" style={{ borderColor: "var(--t-border-dark)", background: "linear-gradient(to bottom, var(--t-inset-from), var(--t-inset-to))" }}>
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-base font-bold tracking-widest truncate" style={{ color: "var(--t-accent)" }}>
              {isPlaying ? currentStation?.name.toUpperCase() : isBuffering ? "CONNEXION..." : hasError ? "⚠ ERREUR SIGNAL" : "FRÉQUENCE GROOVE™"}
            </div>
            <div className="text-xs tracking-wider truncate" style={{ color: "var(--t-text-muted)" }}>
              {isPlaying ? currentStation?.description : isBuffering ? bufferMsg : hasError ? errorMsg : "GunthRadio v2.0 — Qualité Web 1.0 garantie"}
            </div>
          </div>
          {isPlaying && (
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="text-xs tracking-widest" style={{ color: isPlaying ? "var(--t-accent)" : "var(--t-text-subtle)" }}>
        ...
        <div className="flex items-center gap-2">
          ...
          <TrayBtn
            onClick={() => { if (isPlaying) stop(); else if (currentStation) play(currentStation.id as StationId); }}
          ...
          <div
            ...
            style={{ borderTopColor: "var(--t-border-dark)", borderLeftColor: "var(--t-border-dark)", borderBottomColor: "var(--t-border-light)", borderRightColor: "var(--t-border-light)", backgroundColor: "var(--t-app-bg)" }}
            onClick={(e) => {
          ...
          >
            <div className="h-full" style={{ width: `${volume}%`, background: "linear-gradient(to right, var(--t-titlebar-from), var(--t-titlebar-to))" }} />
          </div>
          <span className="text-xs tracking-widest tabular-nums w-8 text-right shrink-0" style={{ color: "var(--t-text-muted)" }}>{volume}%</span>
        </div>
      </div>
        ...
        {STATIONS.map((s) => {
          ...
          return (
            <div
              ...
              style={{ borderColor: "var(--t-border-dark)", backgroundColor: isCurrent ? "var(--t-card-hover)" : "transparent", borderLeft: isCurrent ? "3px solid var(--t-accent)" : "3px solid transparent" }}
              onClick={() => { if (isCurrent && isPlaying) stop(); else play(s.id as StationId); }}
            >
              <div
                ...
                style={{
                  ...
                  backgroundColor: "var(--t-bg)", color: isThisPlaying ? "var(--t-accent)" : "var(--t-text-muted)",
                }}
              >
                {isThisBuffering ? "⏳" : isThisPlaying ? "■" : isThisError ? "⚠" : "▶"}
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-bold tracking-widest truncate" style={{ color: isThisPlaying ? "var(--t-accent)" : "var(--t-text)" }}>
                  {s.emoji} {s.name}
                </div>
                <div className="text-xs tracking-wider truncate" style={{ color: "var(--t-text-muted)" }}>
              ...
              <div
                ...
                style={{ borderColor: isThisPlaying ? "var(--t-accent)" : "var(--t-border-dark)", color: isThisPlaying ? "var(--t-accent)" : "var(--t-text-subtle)", backgroundColor: "var(--t-bg)" }}
              >
                {s.genre}
              </div>
            </div>
      ...
      <div className="shrink-0 px-4 py-1.5 border-t flex justify-between text-xs tracking-widest" style={{ borderColor: "var(--t-border-dark)", color: "var(--t-text-muted)", backgroundColor: "var(--t-bg)" }}>
